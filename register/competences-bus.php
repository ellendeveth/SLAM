<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Document</title>
    <link rel="stylesheet" href="https://use.typekit.net/hbs6zbg.css">
    <link rel="stylesheet" href="../style/style.css">
</head>

<body class="body--startpage">
    <div class="container">
        <div class="container__comp__filters">
            <div class="main">
                <h1 class="form__title">Competenties</h1>
                <span class="subtitle-big">Selecteer jouw skills binnen business</span>
            </div>

            <div class="container__filters filters--business">
                <div class="filter">
                    <button>Leading & organizing</button>
                </div>
                <div class="filter">
                    <button>Design & production</button>
                </div>
                <div class="filter">
                    <button>Marketing</button>
                </div>
                <div class="filter">
                    <button>Communicatie</button>
                </div>
                <div class="filter">
                    <button>Business analytics</button>
                </div>
                <div class="filter">
                    <button>Boekhouden</button>
                </div>
                <div class="filter">
                    <button>Fiscaliteit</button>
                </div>

                <div class="filter__btn">
                <a href="../index.php" class="btn btn-competences" name="competences" >Opslaan</a>
            </div>
            </div>
        </div>
    </div>
<script src="../js/competence.js"></script>
</body>

</html>